...
use serde_yaml::{self};
use crate::job::{airtable, utility};
use polars::prelude::*;
...
pub async fn  fetch_sync(api_url: &str, api_endpoint: &str, auth_token: &str, offset_value: &str)-> Result<serde_json::Value, reqwest::Error>{
...
pub async fn extraction(table:&str,api_url: &str, api_endpoint: &str, auth_token: & str,year:&str) -> Result<(),  Box<dyn std::error::Error>>{
...
pub async fn execute(table: &str, at_type:&str, year:&str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api_endpoint_launch:&str = &config[format!("api_endpoint_{}",at_type)][format!("{}",table)].as_str().ok_or("endpoint not valid")?;
    let auth_token: &str = &config["auth_token"].as_str().ok_or("invalid auth token")?;
    let columns = crate::job::config::at_filtered_columns(format!("{}",table).as_str(),"airtable",format!("{}",year).as_str(),job_config_path)?;
    ...
    let  polars_columns : HashSet<&str>= df.get_columns().into_iter().map(|s|{
        s.name()
    }).collect();

    
    let columns_from_config:HashSet<&str> = columns.keys().map(|column|{
            column.as_str()
    }).collect();
    ...
    let empty_column:Vec<Expr> = difference.iter().map(|name|{
        lit(NULL).alias(name.trim())
    }).collect();
    ...
    let  new_df = df.lazy().with_columns(
        empty_column
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>>  {
  ...
  let endpoints = vec![
      ...
      ("product", vec![
          ...
          //"inspiration", 
          //"onsite_category",
          //"parent_colour_product", 
          //"parent_product", "sub_category"
      ]),
  ];
  ...
  let handles: Vec<_> = job_details
        ...
        .map(|job| {
            tokio::spawn(async move {
                if let Err(err) = timeout(std::time::Duration::from_secs(3600), crate::job::airtable::execute(
